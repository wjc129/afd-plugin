...
Stage 0、Stage 1 顺序为每对 Attention/FFN Rank 的 data 和 input-IDs group
执行一次单元素 HCCL 往返，提前完成 P2P 子通信器的懒初始化，避免 FFN 已进入
首次 `recv`、Attention 仍在编译时触发建链或下发超时。这只增加启动阶段开销，
不进入在线请求路径。
