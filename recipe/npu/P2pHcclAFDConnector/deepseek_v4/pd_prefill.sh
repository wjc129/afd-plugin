...
export HCCL_IF_IP="${PREFILL_HCCL_IF_IP:-${HCCL_IF_IP:-${PREFILL_IP:?Set PREFILL_IP or PREFILL_HCCL_IF_IP in the server-local environment}}}"
...
export TP_SOCKET_IFNAME="${TP_SOCKET_IFNAME:-${NETWORK_INTERFACE:?Set NETWORK_INTERFACE or TP_SOCKET_IFNAME in the server-local environment}}"
